...
QuestionManager::QuestionManager(QMap<uint, Question> questions)
    : questions(questions) {
...
void QuestionManager::addQuestion(const QString &questionContents,
  ...
  if (questionContents.isEmpty()) {
    ...
    questions.insert(questions.isEmpty() ? 0 : questions.lastKey() + 1,
                     Question(questionContents, questionAnswers,
...
void QuestionManager::clearQuestions() noexcept { questions.clear(); }
