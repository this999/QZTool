 ...
 *
 * The QuestionManager class is responsible for managing a collection of
 * questions. It provides functionality to add, remove, edit, and retrieve
 * questions, as well as managing question data in a database or from external
 * files.
 *
 * Example usage:
 *   QuestionManager qm;
 *   qm.addQuestion("What is the capital of Poland?", {"Krakow", "Warsaw",
 * "Gdansk"}, 1);
 *
 */
...
public:
  ...
  void addQuestion(const QString &questionContents,
                   const QStringList &questionAnswers,
                   const uint questionIndexOfCorrectAnswer);
  void removeQuestion(const uint index);
  void changeOneAnswer(const uint indexOfQuestion, const uint indexOfAnswer,
                       const QString &newAnswer);
  void changeAllAnswers(const uint indexOfQuestion,
