...
TEST_F(QuestionTest, functionShouldThrowExceptionTest) {
  ...
  EXPECT_THROW(Question q(contest, emptyList, indexOfCorrectAnswer),
               std::invalid_argument);
  EXPECT_THROW(question.setAnswers(emptyList), std::invalid_argument);
  EXPECT_THROW(Question q(contest, answers, outOfRangeIndex),
               std::out_of_range);
  ...
  EXPECT_THROW(question.setIndexOfCorrectAnswer(outOfRangeIndex),
               std::out_of_range);
